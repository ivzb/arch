...
interface LinksFtsDao {
    ...
    @Query("SELECT rowid, url, sitename, title, imageUrl, category FROM linkFts WHERE rowid = :id")
    fun observe(id: Int): LiveData<LinkFtsEntity>

    @Query("SELECT rowid, url, sitename, title, imageUrl, category FROM linkFts WHERE rowid = :id")
    fun get(id: Int): LinkFtsEntity

    @Query("SELECT rowid, url, sitename, title, imageUrl, category FROM linkFts ORDER BY rowid DESC")
    ...
    @Query("UPDATE linkFts SET sitename = :sitename, title = :title, imageUrl = :imageUrl, url = :url, category = :category WHERE rowid = :id")
    fun update(id: Int, url: String, category: String, sitename: String? = null, title: String? = null, imageUrl: String? = null)
